(defproject pedestal-stylesheets "0.0.1-SNAPSHOT"
  ...
  :aliases {"run-dev" ["trampoline" "run" "-m" "pedestal-stylesheets.server/run-dev"]}
  :repl-options  {:init-ns user
                  :init (try
                          (use 'io.pedestal.service-tools.dev)
                          (require 'pedestal-stylesheets.service)
                          ;; Nasty trick to get around being unable to reference non-clojure.core symbols in :init
                          (eval '(init pedestal-stylesheets.service/service #'pedestal-stylesheets.service/routes))
                          (catch Throwable t
  ...
  :main ^{:skip-aot true} pedestal-stylesheets.server)
